...
public final class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager =
                (ActivityManager) CxaUtil.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = CxaUtil.getContext().getPackageManager();
            ...
            try {// 有"有权查看使用权限的应用"选项
                ApplicationInfo info =
                        packageManager.getApplicationInfo(CxaUtil.getContext().getPackageName(), 0);
                AppOpsManager aom =
                        (AppOpsManager) CxaUtil.getContext().getSystemService(Context.APP_OPS_SERVICE);
                if (aom != null) {
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS,
                            info.uid,
                            info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        CxaUtil.getContext().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS,
                            info.uid,
                            info.packageName) != AppOpsManager.MODE_ALLOWED) {
                ...
                UsageStatsManager usageStatsManager = (UsageStatsManager) CxaUtil.getContext()
    ...
    public static int gc() {
        ...
        int count = 0; // 清理掉的进程数
        ActivityManager am = (ActivityManager) CxaUtil.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        // 获取正在运行的service列表
    ...
    public static boolean isMainProcess() {
        return CxaUtil.getContext().getPackageName().equals(getCurrentProcessName());
    }
